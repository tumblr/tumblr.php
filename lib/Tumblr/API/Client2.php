...
/**
 * A client to access the Tumblr API with OAuth2 credentials
 */
class Client2
{
    /**
     ...
     * @param array $options
     */
    public function __construct($access_token, $options=[])
    {
     ...
     */
    public function getRequest($path, $options)
    {
        if (!$options) {
            $options = [];
        }
    ...
    public function postRequest($path, $options)
    {
        ...
        $response = $this->makeRequest('POST', $path, $options);
        return $this->parseResponse($response);
    }
     ...
     */
    private function getRedirect($path, $options)
    {
        $response = $this->makeRequest('GET', $path, $options);
